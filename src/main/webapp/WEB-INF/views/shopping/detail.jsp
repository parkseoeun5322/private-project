	...
	function go_push() {
		...
		if( $thumbsUp.children().attr("data-prefix") == "fas" ) {
			if(confirm("이미 추천한 게시글입니다.\n추천을 취소하시겠습니까?")) {
				$.ajax({
					url: "push_cancel",
					data: { push_boardNo: ${vo.shopping_no}, 
	...
	function go_scrap() {
		...
		if("${scrap}" == "") {
			if(confirm("스크랩하시겠습니까?")) {
				$.ajax({
					url: "scrap",
					data: { scrap_boardNo: ${vo.shopping_no},
							scrap_category: "${vo.board_category}",
							scrap_title: "${vo.shopping_title}" },
					success:function(data) {
						if(data) {
							window.location.reload();
						} else {
...
<body>
	<div class="detail_style">
		<ul>
			<li>
				...
				<div><a href="">${vo.shopping_header })</a>${vo.shopping_title }</div>
				<div>${vo.shopping_writedate }</div>
			</li>
			<li>
				<div>
					<span>조회수<b>${vo.shopping_readcnt }</b></span>
					<span id="pushCnt">추천수<b>${vo.shopping_pushcnt }</b></span>
			...
			<li>
				...
				<c:if test="${login_info ne null }">
					<div class="fr">
						...
						<!-- <i class="fas fa-thumbs-up"></i> -->
						<span><a onclick="go_scrap()">스크랩<b>${vo.shopping_scrapcnt }</b></a></span>
					</div>
				</c:if>
				<c:if test="${login_info.member_id eq vo.shopping_writer }">
					<div class="fr"><a onclick="$('#pageForm').attr('action', 'modify.tv'); $('#pageForm').submit()">수정</a></div>
					<div class="fr"><a onclick="if(confirm('정말 삭제하시겠습니까?')) { $('#pageForm').attr('action', 'delete.tv'); $('#pageForm').submit() }">삭제</a></div>
				</c:if>
			</li>
		</ul>
		<form id="pageForm" action="list.tv" method="post">
	...
	<script type="text/javascript">
		...
		function comment_list() {
			$.ajax({
				url: "board/comment/${vo.shopping_no}",
				data: { board_writer: "${vo.shopping_writer}",
